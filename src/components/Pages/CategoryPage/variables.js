export const RESTAURANTS = [
  {
    id: 1,
    img: "https://img.freepik.com/fotos-kostenlos/eine-restauranthalle-mit-holztischen-und-rohren-aus-rotem-backstein-an-der-decke_140725-8504.jpg",
    title: "Яккачинор",
    description:
      "Ресторан Яккачинор предлагает широкий выбор блюд узбекской кухни, уютную атмосферу и профессиональное обслуживание. Идеальное место для проведения свадебных торжеств.",
    price: 50000,
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "restaurant",
  },
  {
    id: 2,
    img: "https://ik.imagekit.io/tuyona/Restaurants/%D0%AF%D0%BA%D0%BA%D0%B0%D1%81%D0%B0%D1%80%D0%BE%D0%B9%20/maxresdefault.jpg?updatedAt=1754731707791",
    title: "Яккасарой",
    description:
      "Ресторан Яккачинор предлагает широкий выбор блюд узбекской кухни, уютную атмосферу и профессиональное обслуживание. Идеальное место для проведения свадебных торжеств.",
    price: 10000,
    imgList: [
      "https://ik.imagekit.io/tuyona/Restaurants/%D0%AF%D0%BA%D0%BA%D0%B0%D1%81%D0%B0%D1%80%D0%BE%D0%B9%20/XXL_height.webp?updatedAt=1754731707719",
      "https://ik.imagekit.io/tuyona/Restaurants/%D0%AF%D0%BA%D0%BA%D0%B0%D1%81%D0%B0%D1%80%D0%BE%D0%B9%20/h-_qtcOlIrQ.jpg?updatedAt=1754731707913",
      "https://ik.imagekit.io/tuyona/Restaurants/%D0%AF%D0%BA%D0%BA%D0%B0%D1%81%D0%B0%D1%80%D0%BE%D0%B9%20/5faa5ad47e17f300cac7d50a82550419ff0b195b_x3.jpg?updatedAt=1754731707530",
    ],
    type: "restaurant",
  },
];

export const MUSICIANS = [
  {
    id: 1,
    img: "https://img.freepik.com/free-photo/beautiful-brunette-woman-singing-microphone-isolated-gray-wall_231208-12884.jpg?semt=ais_hybrid&w=740",
    title: "Группа Парем",
    description:
      "Восточная группа Парем предлагает живую музыку для свадебных мероприятий. Профессиональные музыканты создадут атмосферу праздника и веселья.",
    price: "Договорная цена",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "musicians",
  },
];

export const CARS = [
  {
    id: 1,
    img: "https://www.myviplimo.de/wp-content/uploads/2019/05/Limousinenservice31.jpg",
    title: "Моши Люкс",
    description:
      "Прокат автомобилей Моши Люкс предлагает широкий выбор автомобилей для свадебных мероприятий. Профессиональные водители и комфортные автомобили сделают ваш день особенным.",
    price: "Договор",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "cars",
  },
];

export const DECORATION = [
  {
    id: 1,
    img: "https://www.we-de.ru/getpic.php?h=450&src=/upload/iblock/e34/e34ebff1662d4ac5830a7251fc5d128c.jpg",
    title: "Декорация Свадеб",
    description:
      "Декорация Свадеб предлагает профессиональные услуги по оформлению свадебных мероприятий. Красивые цветочные композиции, оформление залов и многое другое для создания незабываемой атмосферы.",
    price: "1000-5000",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "decoration",
  },
];

export const PRESENTERS = [
  {
    id: 1,
    img: "https://lh3.googleusercontent.com/pHbu0l1XY_RTyHvH-ZAkScnBKie4HzJXZYYRBOFkmu3Sl3fbVLX3Yar5tyfQcKF-xsnKwdlw7H51Gw3_haNTrgU-Kt_QPe_fIyZS-Q=w961-h641-l80-e31",
    title: "Игорь крутой",
    description:
      "Игорь Крутой - профессиональный ведущий свадебных мероприятий. С его харизмой и опытом ваш свадебный день станет незабываемым.",
    price: "5000",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "presenters",
  },
];

export const PHOTOGRAPHERS = [
  {
    id: 1,
    img: "https://static.alltime.ru/obj/article/image-blog/Fotograf_na_svadbu/Fotograf_na_svadbu_1.jpg",
    title: "Суратои киноги",
    description:
      "Суратои киноги - профессиональный фотограф, специализирующийся на свадебной фотографии. Он запечатлеет самые важные моменты вашего дня с высоким качеством и креативным подходом.",
    price: "5000",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "photographers",
  },
];

export const SINGERS = [
  {
    id: 1,
    img: "https://mp3.tj/img/artist/b/8/4/623.jpg",
    title: "Чонибек Муродов",
    description:
      "Чонибек Муродов - популярный певец, который украсит ваш свадебный день своими песнями. Его голос и харизма создадут атмосферу праздника и радости.",
    price: "Договор",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "singers",
  },
];


export const BEAUTY_SALON = [
  {
    id: 1,
    img: "https://cdn.pixabay.com/photo/2019/03/08/20/17/beauty-salon-4043096_1280.jpg",
    title: "Салон Красоты",
    description:
      "Салон Красоты предлагает профессиональные услуги по прическам и макияжу для невест. Опытные стилисты помогут создать идеальный образ для вашего свадебного дня.",
    price: "3000",
    imgList: [
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
      "https://img.freepik.com/free-photo/beautiful-brun",
    ],
    type: "beautySalons",
  },
];
