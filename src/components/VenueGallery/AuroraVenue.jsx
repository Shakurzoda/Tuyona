...
/** ЕДИНЫЙ нормализатор медиа + защита от дублей */
...
function Tile({ item, className, onClick }) {
    ...
    const videoSrc = item.sources?.[0]?.src || "";

    // Видео: показываем сам <video> как превью кадра
    return (
        <button
...
function SvgSocial({ name }) {
    const icons = {
        ...
        whatsapp: (
            <svg viewBox="0 0 24 24" className={s.socialIcon} aria-hidden>
                <path d="M20 3.9A10 10 0 1 0 4.2 19.7L3 23l3.4-1.2A10 10 0 1 0 20 3.9zM7.5 9.4c.2-.5.4-.6.8-.6h.6c.2 0 .5.1.6.4л.5 1.1c.1.3.1.5-.1.7л-.5.6c.6 1.1 1.6 2 2.7 2.7л.6-.5c.2-.2.4-.2.7-.1л1.1.5c.3.1.4.4.4.6v.6c0 .4-.2.6-.6.8-.3.1-.9.3-1.9.1-1.7-.4-3.8-2.4-4.5-4.1-.4-1-.2-1.6 0-1.8z" />
            </svg>
        ),
        telegram: (
            <svg viewBox="0 0 24 24" className={s.socialIcon} aria-hidden>
                <path d="M21.5 3.5 3 10.6c-1.2.5-1.2 2.2.1 2.6л4.3 1.4 1.6 5.1c.3 1 1.6 1.2 2.2.4л2.8-3.5 4.5 3.3c.9.7 2.2.2 2.5-1л3-14c.3-1.3-1-2.4-2.5-1.8z" />
            </svg>
        ),
        instagram: (
            <svg viewBox="0 0 24 24" className={s.socialIcon} aria-hidden>
