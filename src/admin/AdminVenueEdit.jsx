...
export default function AdminVenueEdit() {
  ...
  const save = async () => {
    ...
    // ВАЖНО: не отправляем id в payload,
    // чтобы не пытаться обновлять identity-колонку
    const { id, ...venueWithoutId } = venue;
    const payload = {
      ...venueWithoutId,
      type: normalizeType(venue.type),
    };
    ...
    // Перемещаем драфт-файлы в постоянный путь
    ...
    if (normalizedMedia.length) {
      await supabase.from("venue_media").insert(
        normalizedMedia.map((m, i) => ({
          venue_id: currentId,
          kind: m.kind,
          src: sanitizeMediaUrl(m.src),
          poster: sanitizeMediaUrl(m.poster) || null,
          sort_order: i,
          path: m.path || null,
